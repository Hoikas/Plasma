...
def dump_exc(what: str, output: io.IOBase = sys.stderr) -> str:
    global exitcode
    exitcode = 1
    ...
    try:
        ...
        output.write("Crap, I couldn't figure this one out!\n")
        output.write(result)
        output.write("\n")
        sys.exit(1)
...
def dump_missing_id(filename: Path, output: io.IOBase = sys.stderr):
    global exitcode
    exitcode = 1
...
def dump_dupe_id(output: io.IOBase = sys.stderr):
    global exitcode
