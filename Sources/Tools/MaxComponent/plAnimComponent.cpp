...
{
    const MCHAR* name = fCompPB->GetStr(kAnimName);
    if (!name || name[0] == _M('\0'))
        return ST::string();
    return ST::string(name);
}
...
//  {
        ...
        if (mod == nullptr)
        {
            ...
            {
                node->AddModifier(new plAGModifier(ST::string(name)), IGetUniqueName(node));
            }
...
//  }
    ...
    ST::string animName = ST::string(fCompPB->GetStr(kAnimName));
    if (animName.empty())
    ...
    else
    {
        ...
        if (fCompPB->GetInt(kAnimLoop))
        {
            ...
            ST::string loopName = ST::string(fCompPB->GetStr(kAnimLoopName));
...
bool plAnimComponentBase::IAddTMToAnim(plMaxNode *node, plAGAnim *anim, plErrorMsg *pErrMsg)
{
    ...
    if (tmc)
    {
        plMatrixChannelApplicator *app = new plMatrixChannelApplicator();
            app->SetChannelName(ST::string(node->GetName()));
...
bool plAnimComponentBase::IConvertNodeSegmentBranch(plMaxNode *node, plAGAnim *anim, plErrorMsg *pErrMsg)
{
    ...
    if (madeAnim)
    {
        ...
        if (!node->HasAGMod())
        {
            node->AddModifier(new plAGModifier(ST::string(node->GetName())), IGetUniqueName(node));
        }
        madeAnim = true;
    }
/*
