...
bool plMoviePlayer::Start()
{
    ...
    fTexture = new plMipmap(static_cast<uint32_t>(video->GetWidth()), static_cast<uint32_t>(video->GetHeight()), plMipmap::kGray4Config);
    fTexture->fCompressionType = plBitmap::kUncompressed;
    fTexture->fUncompressedInfo.fType = plBitmap::UncompressedInfo::kInten8;
    hsgResMgr::ResMgr()->NewKey(plString::Format("WebM Movie - %s", fMoviePath.c_str()), fTexture, plLocation::kGlobalFixedLoc);
    fPlate->SetTexture(fTexture);
    return true;
#else
    return false;
#endif // VPX_AVAILABLE
